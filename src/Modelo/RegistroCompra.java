...
public class RegistroCompra extends RegistroBD{
     ...
     public ArrayList<Compra> consultarCompra() {
        ...
        try {
            ...
            while (resultado.next()) {
                listaCompra.add(new Compra (resultado.getInt("id_producto"),resultado.getInt("precio del producto"),resultado.getInt("cantidad de productos"),resultado.getString(null)
                        ,resultado.getString(null),resultado.getString(null),resultado.getInt(null),resultado.getInt(null)));
            }
    ...
    public Compra verificarIdProducto(int idPro){
       try {
        ...
        resultado= this.consulta(sql);
        while (resultado.next()){
            return new Compra(resultado.getInt("id_producto"),resultado.getInt("precio del producto"),resultado.getInt("cantidad de productos"),resultado.getString(null)
                    ,resultado.getString(null),resultado.getString(null),resultado.getInt(null),resultado.getInt(null));
        }
        } catch (SQLException ex) {
            Logger.getLogger(RegistroProveedores.class.getName()).log(Level.SEVERE, null, ex);
