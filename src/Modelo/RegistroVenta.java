...
public class RegistroVenta extends RegistroBD {
    ...
    public void disminuirStock() throws SQLException {
        ArrayList<String> codigos = new ArrayList<String>();
        ArrayList<Integer> cantidades = new ArrayList<Integer>();
        String[] cods;
        sql = "select id_producto, cantidad from tb_factura where id_venta= " + "(select max(id_Venta) from(select * from tb_venta)as x);";
        resultado = this.consulta(sql);
        while (resultado.next()) {
            codigos.add(resultado.getString("id_producto"));
            cantidades.add(resultado.getInt("cantidad"));
        }
        cods = new String[codigos.size()];
        cods = codigos.toArray(cods);
        for (int i = 0; i < cods.length; i++) {
            sql = "update tb_productos set cantidad=(select cantidad from(select * from tb_productos where id_producto= " + cods[i] + ")as x)-" + cantidades.get(i) + " where id_producto=" + cods[i] + ";";
            this.proceso(sql);
        }
